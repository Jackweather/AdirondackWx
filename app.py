...
def run_task():
    def run_all_scripts():
        ...
        try:
            subprocess.run(["python", "temp2m.py"], check=True)
            print("temp2m_script.py ran successfully!")
        except subprocess.CalledProcessError:
            error_trace = traceback.format_exc()
            print(f"Error running temp2m_script.py:\n{error_trace}")

        try:
            subprocess.run(["python", "LIGHTNING.py"], check=True)
            print("lightning_script.py ran successfully!")
        except subprocess.CalledProcessError:
            error_trace = traceback.format_exc()
            print(f"Error running lightning_script.py:\n{error_trace}")
